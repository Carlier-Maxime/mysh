...
void sort_dir_path_tab(char** dir_path_tab, int size){
	for(int i=0;i<size;i++){
		...
		for(int j=i+1;j<size;j++){
			if(strcmp(get_name(dir_path_tab[min_idx]),get_name(dir_path_tab[j]) )>0) min_idx=j;
		}
		if(i!=min_idx){
...
void sort_file_tab(full_file* file_tab, int size){
	for(int i=0;i<size;i++){
		int min_idx=i;
		for(int j=i+1;j<size;j++){
			if(strcmp(get_name(file_tab[min_idx].name),get_name(file_tab[j].name))>0) min_idx=j;
		}
		if(i!=min_idx){
